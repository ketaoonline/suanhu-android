...
//    SwipeMenuRecyclerView rv;
    ...
    protected String setTitleStr() {
        ...
        return detailedTest.getUser_name();
    }
...
//        FinishChatActivity.open(this, 1000);
    }
    ...
    private void sendMsg(String imagUrl) {
        ...
        Http.http.createApi(MineAPI.class)
                .sendMsg(detailedTest.getOrder_use_sn(), detailedTest.getUser_id(), type, content, lastMsgId)
...
//                .cropCompressQuality(10)
    ...
    private void getUserInfo() {
        Http.http.createApi(MineAPI.class)
                .getCustomerInfo(detailedTest.getOrder_use_sn())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribeOn(Schedulers.newThread())
                .subscribe(new AppSubscriber(new RequestCallBack<CustomerInfo>() {
                    @Override
                    public void success(CustomerInfo result) {
                        tvName.setText("客户姓名："+result.getUser_name());
                        tvBirth.setText("客户生辰："+result.getBazi());
                        tvType.setText("咨询类型："+result.getChat_type());
                    }
    ...
    private void getHistory(final boolean isLoadMore) {
        Http.http.createApi(MineAPI.class)
