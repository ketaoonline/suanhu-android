...
public class IDCardValidate {
     ...
     * @param idCardNum
     */
    public IDCardValidate(String idCardNum) {
        // super();
    ...
    public int isCorrect() {
        if (this.isEmpty()) {
            this.error = IDCardValidate.IS_EMPTY;
            return this.error;
        }

        if (!this.isLenCorrect()) {
            this.error = IDCardValidate.LEN_ERROR;
            return this.error;
        }

        if (!this.isCharCorrect()) {
            this.error = IDCardValidate.CHAR_ERROR;
            return this.error;
        }

        if (!this.isDateCorrect()) {
            this.error = IDCardValidate.DATE_ERROR;
            return this.error;
        }

        if (this.is18()) {
            if (!this.getCheck().equals(this.getCheckBit())) {
                this.error = IDCardValidate.CHECK_BIT_ERROR;
                return this.error;
            }
        }
