package com.xiaoxiong.library.utils.animation;
...
public class JAnimationUtil {
    ...
    public static void startExitViewScaleAnim(Context context,View target, float originalScale, PhotoInfoPosition infos, Animator.AnimatorListener animatorListener){
        ...
        if(imgScale >= windowScale){
            animImgStartHeight = JWindowUtil.getWindowHeight(context) * originalScale;
            pivotX = localX / (1 - originalScale);
            pivotY = (localY - (animImgStartHeight - height) / 2) / (1 - originalScale);
        }else{
            animImgStartWidth = JWindowUtil.getWindowWidth(context) * originalScale;
        ...
        ObjectAnimator animatorTransX = ObjectAnimator.ofFloat(target, View.TRANSLATION_X,
                target.getTranslationX() +
                        (JWindowUtil.getWindowWidth(context) / 2 * (1 - target.getScaleX()) -
        ...
        ObjectAnimator animatorTransY = ObjectAnimator.ofFloat(target, View.TRANSLATION_Y,
                target.getTranslationY() +
                        (JWindowUtil.getWindowHeight(context) / 2 * (1 - target.getScaleY()) -
    ...
    public static void startEnterViewScaleAnim(Context context,View target, float originalScale, PhotoInfoPosition infos, Animator.AnimatorListener animatorListener){
