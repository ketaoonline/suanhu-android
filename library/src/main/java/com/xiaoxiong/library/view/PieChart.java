...
public class PieChart extends View {
    ...
    public void initSrc(float[] item, String[] colors, OnItemClickListener listener){
        boolean isAllZero = isAllZero(item);
        if (isAllZero) {
            item = new float[]{1.0f};
            colors = new String[]{"#909090"};
        }
        setItem(item);
        setColors(colors);
        notifyDraw();
     ...
     * @param colors
     */
    public void initSrc(int densiTyDpi,float[] item,String[] colors){
        radius = (float) 0.41 * densiTyDpi;
        boolean isAllZero = isAllZero(item);
        if (isAllZero) {
            item = new float[]{1.0f};
            colors = new String[]{"#909090"};
...
//	}
    ...
    private boolean isAllZero(float[] datas){
